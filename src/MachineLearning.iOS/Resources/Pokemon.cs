...
namespace MachineLearning.iOS {
	/// <summary>
	/// Model Prediction Input Type
	/// </summary>
	public class PokemonInput : NSObject, IMLFeatureProvider
	{
		...
		public PokemonInput (CVPixelBuffer data)
		{
			if (data == null)
	...
	/// </summary>
	public class PokemonOutput : NSObject, IMLFeatureProvider
	{
		static readonly NSSet<NSString> featureNames = new NSSet<NSString> (
		...
		public PokemonOutput (NSDictionary<NSObject, NSNumber> loss, string classLabel)
		{
			if (loss == null)
	...
	public class Pokemon : NSObject
	{
		...
		public Pokemon ()
		{
			var url = NSBundle.MainBundle.GetUrlForResource ("Pokemon", "mlmodelc");
			NSError err;

			model = MLModel.Create (url, out err);
		}

		Pokemon (MLModel model)
		{
			this.model = model;
		}

		public static Pokemon Create (NSUrl url, out NSError error)
		{
			if (url == null)
